...
SEED_FACTS = (
    ...
    "The correct weekly pipeline order is: (1) gladen_html_scraper.py — update PROMO_PERIOD constant first; "
    "(2b) billa_scraper.py — auto-merges to master, no flags needed; "
    "(2a) billa_pdf_pipeline.py — adds PDF-only supplementary items, OCR cache auto-invalidates on new PDF; "
    "(3) write_glovo_data.py — update PROMO_PERIOD and KAUFLAND_FILE path (FireCrawl scrape of "
    "kaufland.bg/aktualni-predlozheniya/ot-ponedelnik.html) before running; "
    "(4) fantastico_pipeline.py — fully automated; "
    "(5) generate_cheapest_xlsx.py — generates Excel report. "
    "Step 2b must run before 2a so the PDF pipeline sees current ssbbilla data for comparison. "
    "secrets.py contains ALL Azure keys and is excluded from git: "
    "AZURE_KEY for Azure Document Intelligence (used by billa_pdf_pipeline.py and fantastico_pipeline.py), "
    "AZURE_OPENAI_KEY for Azure OpenAI GPT-4o (used by generate_cheapest_xlsx.py and translator.py). "
    "azure_secrets.json is a legacy fallback only — secrets.py takes precedence. "
    "Glovo sources (Kaufland/Billa/Fantastico Glovo) are blocked from non-BG IP — require VPN. "
    "Last pipeline run: CW19 (07.05-13.05.2026), 2,734 total records. "
    "The weekly run log is in RUN_LOG.md. "
